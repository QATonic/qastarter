...
import { CheckCircle2, FileText, RefreshCw, Copy, Terminal } from 'lucide-react';
...
export default function SuccessView({ config, onGenerateAnother }: SuccessViewProps) {
  ...
  const getCommands = (): CommandStep[] => {
    ...
    if (lang === 'typescript' || lang === 'javascript') {
      ...
      // Kotlin uses same build tools as Java
      if (build === 'gradle') {
        steps.push({ label: 'Build project', command: './gradlew build' });
        steps.push({ label: 'Run tests', command: './gradlew test' });
      } else {
        steps.push({ label: 'Build project', command: 'mvn clean install' });
        steps.push({ label: 'Run tests', command: 'mvn test' });
      }
    } else if (lang === 'dart') {
