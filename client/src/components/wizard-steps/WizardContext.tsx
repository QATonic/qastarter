...
export function WizardProvider({ children, onComplete, onBack }: WizardProviderProps) {
      ...
      case 6: {
        ...
        if (needsJavaMetadata) {
          ...
          if (!groupId || !/^[a-zA-Z][a-zA-Z0-9]*(\.[a-zA-Z][a-zA-Z0-9]*)*$/.test(groupId))
            return false;
          // artifactId is auto-derived from projectName — no separate validation needed
        }
        return true;
      }
      case 7:
  ...
  const updateConfig = <K extends keyof WizardConfig>(key: K, value: WizardConfig[K]) => {
    ...
    if (key === 'testingType') {
      ...
      newConfig.baseUrl = urlDefaults[value] || '';

      // Web: pre-fill SauceDemo credentials
      if (value === 'web') {
        newConfig.testUsername = 'standard_user';
        newConfig.testPassword = 'secret_sauce';
      } else {
        newConfig.testUsername = '';
        newConfig.testPassword = '';
      }

      // Mobile/Desktop: set sensible app-path defaults
      if (value === 'desktop') {
        newConfig.appPath = 'Microsoft.WindowsCalculator_8wekyb3d8bbwe!App';
      } else {
        newConfig.appPath = '';
      }
      newConfig.deviceName = '';
      newConfig.platformVersion = '';
    ...
    if (key === 'baseUrl' && config.testingType === 'web') {
      const isSauceDemo = /saucedemo\.com/i.test(value);
      if (isSauceDemo) {
        newConfig.testUsername = 'standard_user';
        newConfig.testPassword = 'secret_sauce';
      } else if (config.testUsername === 'standard_user' || !config.testUsername) {
        // Only clear if they haven't manually typed something
        newConfig.testUsername = '';
        newConfig.testPassword = '';
      }
    }
