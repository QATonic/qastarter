...
test.describe('QAStarter API', () => {
  ...
  test('POST /api/project-preview should return file structure', async ({ request }) => {
    const response = await request.post('/api/v1/project-preview', {
      data: {
        testingType: 'web',
  ...
  test('POST /api/generate-project should return zip file', async ({ request }) => {
    const response = await request.post('/api/v1/generate-project', {
      data: {
        testingType: 'web',
        framework: 'playwright',
        language: 'typescript',
  ...
  test('POST /api/validate-config should validate configuration', async ({ request }) => {
    // Valid config
    const validResponse = await request.post('/api/v1/validate-config', {
      data: {
        testingType: 'web',
        framework: 'playwright',
        language: 'typescript',
    ...
    // Invalid config
    const invalidResponse = await request.post('/api/v1/validate-config', {
      data: {
        testingType: 'web',
        framework: 'cypress',
  ...
  test('POST /api/validate-config with incompatible framework/language combo', async ({
    ...
    const response = await request.post('/api/v1/validate-config', {
      data: {
        testingType: 'api',
  ...
  test('GET /api/v1/config/options should return complete config data', async ({ request }) => {
    const response = await request.get('/api/v1/config/options');
    expect(response.ok()).toBeTruthy();
  ...
  test('POST /api/generate-project for different testing types', async ({ request }) => {
    ...
    const apiResponse = await request.post('/api/v1/generate-project', {
      data: {
        testingType: 'api',
        framework: 'restassured',
        language: 'java',
    ...
    const mobileResponse = await request.post('/api/v1/generate-project', {
      data: {
        testingType: 'mobile',
        framework: 'appium',
        language: 'java',
