...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/generate-project", generateProjectLimiter, async (req, res) => {
    ...
    try {
      ...
      if (!WizardValidator.isCompatible(config.testingType, config.framework, config.language)) {
        console.error(`[${requestId}] Incompatible combination: ${config.testingType} + ${config.framework} + ${config.language}`);
        return res.status(400).json({
          success: false,
      ...
      const totalDuration = Date.now() - startTime;
      console.error(`[${requestId}] ❌ Project generation failed:`, {
  ...
  app.get("/api/v1/config/options", (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message: "Error retrieving configuration options"
      });
    }
  });
  ...
  app.get("/api/v1/config/filters", (req, res) => {
    try {
      ...
      res.json({
        ...
        data: filters,
        timestamp: new Date().toISOString(),
        requestId: `filters-${Date.now()}`
      });
    } catch (error) {
      console.error('Filter rules error:', error);
      res.status(500).json({
        success: false,
        message: "Error retrieving filter rules"
      });
    }
  });
  ...
  app.post("/api/project-dependencies", async (req, res) => {
    try {
      // Validate request body
      const validationResult = projectConfigSchema.safeParse(req.body);
      ...
      res.status(500).json({
        success: false,
        message: "Error fetching project dependencies"
      });
    }
  });
  ...
  app.post("/api/project-preview", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message: "Error generating project preview"
      });
    }
  });
  ...
  app.get("/api/v1/generate", generateProjectLimiter, async (req, res) => {
    const startTime = Date.now();
    const requestId = `api-gen-${Date.now()}-${Math.random().toString(36).substring(7)}`;
    ...
    try {
      ...
      if (!validationResult.success) {
        ...
        return res.status(400).json({
          ...
          message: "Invalid configuration",
          errors: validationResult.error.errors,
          hint: "Use GET /api/v1/metadata to see available options"
        });
      }
      ...
      const totalDuration = Date.now() - startTime;
      console.error(`[${requestId}] ❌ Public API project generation failed:`, {
        durationMs: totalDuration,
        error: error instanceof Error ? error.message : 'Unknown error',
        stack: error instanceof Error ? error.stack : undefined
      });
