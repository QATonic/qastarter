...
export const projectConfigSchema = z.object({
  ...
  projectName: z.string().min(1).max(100).regex(/^[a-zA-Z0-9_-]+$/, "Project name can only contain letters, numbers, hyphens, and underscores"),
  groupId: z.string().optional().refine(val => !val || /^[a-zA-Z0-9._-]+$/.test(val), "Group ID can only contain letters, numbers, dots, hyphens, and underscores"),
  artifactId: z.string().optional().refine(val => !val || /^[a-zA-Z0-9_-]+$/.test(val), "Artifact ID can only contain letters, numbers, hyphens, and underscores"),
